...
uint32_t LoraSx1262::frequencyToPLL(long rfFreq) {
  ...
  //return (uint32_t)((double)rfFreq / (double)((double)13000000 / (double)pow(2.0, 25.0)));
...
uint16_t LoraSx1262::getErrors() {
  ...
  RadioError_t error;
	...
	SX126xReadCommand(0x17, (uint8_t *)&error, 2);
  //error.Value = errors;
  Serial.println("Status Flags:");
  Serial.println("-------------------");
  Serial.print("Rc64kCalib: "); Serial.println(error.Fields.Rc64kCalib ? "FAIL" : "OK");
  Serial.print("Rc13mCalib: "); Serial.println(error.Fields.Rc13mCalib ? "FAIL" : "OK");
  Serial.print("PllCalib:   "); Serial.println(error.Fields.PllCalib ? "FAIL" : "OK");
  Serial.print("AdcCalib:   "); Serial.println(error.Fields.AdcCalib ? "FAIL" : "OK");
  Serial.print("ImgCalib:   "); Serial.println(error.Fields.ImgCalib ? "FAIL" : "OK");
  Serial.print("XoscStart:  "); Serial.println(error.Fields.XoscStart ? "FAIL" : "OK");
  Serial.print("PllLock:    "); Serial.println(error.Fields.PllLock ? "FAIL" : "OK");
  Serial.print("BuckStart:  "); Serial.println(error.Fields.BuckStart ? "FAIL" : "OK");
  Serial.print("PaRamp:     "); Serial.println(error.Fields.PaRamp ? "FAIL" : "OK");
  Serial.println("-------------------");
  Serial.print("Raw Value: 0x"); Serial.println(error.Value, HEX);

  // digitalWrite(SX1262_NSS,0); //Enable radio chip-select
  // spiBuff[0] = 0x07;  //Opcode for set RF Frequencty
  // spiBuff[1] = 0x00;  //MSB of pll frequency
  // SPI.transfer(spiBuff,2);
  // digitalWrite(SX1262_NSS,1); //Disable radio chip-select
  // waitForRadioCommandCompletion();  
